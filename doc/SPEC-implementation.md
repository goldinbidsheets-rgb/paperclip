...
- `blockerAttention` is an advisory liveness projection: `unknown` with reason `truncated` or `cycle_detected` means the traversal was not computable and must never be treated as a live blocker; `blockedBy` remains the first-class graph, and `unresolvedBlockerCount` counts unresolved first-class blocker relations rather than child paths
